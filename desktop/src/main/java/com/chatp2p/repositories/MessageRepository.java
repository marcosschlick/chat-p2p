package com.chatp2p.repositories;

import com.chatp2p.managers.DatabaseManager;
...
public class MessageRepository {
    ...
    public void save(Message msg, Long senderId, Long recipientId) {
        if (msg.getType() != MessageType.TEXT && msg.getType() != MessageType.FILE) {
            return;
        }
        ...
        try (PreparedStatement ps = DatabaseManager.getConnection().prepareStatement(sql)) {
            ...
            ps.setString(4, msg.getType() == MessageType.FILE ? msg.getFileName() : msg.getContent());
            ps.executeUpdate();
    ...
    public List<Message> findHistory(Long userA, Long userB) {
        ...
        String sql = "SELECT sender_id, type, content, created_at FROM messages " + "WHERE (sender_id = ? AND recipient_id = ?) OR (sender_id = ? AND recipient_id = ?) " + "ORDER BY created_at ASC";
